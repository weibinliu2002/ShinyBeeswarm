...
.libPaths("D:/Program Files/R/lib")
      ...
      tabPanel("Corrplot",
       tabPanel("Chord Diagram",
        sidebarPanel(
         ...
         conditionalPanel(condition = "input.shape",
         sliderInput("slider1",label = "图形大小",min = 0,max = 5,value = 1,step=0.1),
         selectInput("select6",label = "形状",choices = list("圆形"=16,"三角形"=17,"菱形"=18,"正方形"=15))),
         checkboxInput("others", "方向", FALSE) ,
         conditionalPanel(condition = "input.others",
         selectInput("select7",label = "朝向",choices = list("默认"=0,"向右"=1,"向左"=-1)),
         selectInput("select8",label = "method ",choices = list("swarm","square","hex","center")),
         selectInput("select9",h4("priority:",
         ...
         selectInput("select10",h4("corral:",
                                  bsButton("bs1", label = "", icon = icon("question"), style = "info", size = "small")
         ),choices = list("omit","random","wrap","gutter","none")),
         bsPopover("bs1", "当method选为swarm并且priority选为random时，可以更改corral。", trigger = "focus")),
         ...
         conditionalPanel(
           ...
           selectInput("select1",label = "字体位置",choices = list("标题"=1,"横坐标"=2,"纵坐标"=3,"横坐标标题"=4,"纵坐标标题"=5)),
           ...
           conditionalPanel(condition="input.select1 == '2'",
                            ...
                            radioButtons("t1",label=h4("字体样式"),
                                         choices = list("sans", "serif", "mono", "wqy-microhei" ,"STKaiti", "simhei" )),
                            sliderInput("slider4",label = "横坐标字体大小",min = 5,max = 40,value = 10)),
           ...
           conditionalPanel(condition="input.select1 == '4'",
                            radioButtons("f4",label=h4("字体粗斜"),
                                         choices = list("默认"="plain","粗体"="bold","斜体"="italic","粗斜体"="bold.italic")),
                            radioButtons("t4",label=h4("字体样式"),
                                         choices = list("sans", "serif", "mono", "wqy-microhei" ,"STKaiti", "simhei" )),
                            sliderInput("slider5",label = "横坐标标题字体大小",min = 1,max = 40,value = 10)),
...
output$p1 <- renderPlot({
  ggplot(data, aes(x = Groups, y = Value ))+ 
    ...
    geom_beeswarm(cex = 1,size=input$slider1,method = input$select8,
                  priority = input$select9,corral=input$select10,color = Group2,
    ...
    theme(legend.position = 'none',
          ...
          axis.title.x = ggplot2::element_text(size = input$slider5,face=input$f4,vjust = 3,family = input$t4),
          ...
          axis.text.x = element_text(family = input$t1, size = input$slider4, color="black",angle=0,face = input$f1),
...
observe({
  ...
  output$Download1 <- downloadHandler(
    ...
    content <- function(file) {
      ...
      write.table(exampl, file = file,row.names = F,col.names = T,quote = F
      ) 
